...
bool Maze::isSolved(Cell* curr_cell, StackLinked<Cell>* stack)
{
   ...
   if (col == width && row == height && maze->getElement(row, col) == 1)
	{
...
bool Maze::traverse()
{
   ...
   while(!stack.isEmpty())                                                                                                                                               //Start the maze maze searching process
   {
      Cell* top_cell = processBackTrack(&stack);																												//Current cell == the cell returned from backtrack
      ...
      {
			//DO THIS
			delete curr_cell; //delete the cell
